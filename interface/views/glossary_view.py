...
class GlossaryView:
    ...
    def render(self):
        ...
        if not glossary_path.exists():
            ...
            st.info(
                "Please create the glossary.json file in the interface/static/ directory."
            )
            return
        ...
        if total_items > 0:
            st.caption(
                f"Showing {start_idx + 1}-{end_idx} of {total_items} term(s) | Page {st.session_state.glossary_page} of {total_pages}"
            )
        else:
        ...
        if not filtered_terms:
            ...
            if total_pages > 1:
                ...
                with col_info:
                    st.markdown(
                        f"<div style='text-align: center; padding-top: 8px;'>Page {st.session_state.glossary_page} of {total_pages}</div>",
