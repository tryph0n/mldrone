...
Uses test samples from data/test_samples/{CLEAN,WIFI,BLUE,BOTH}/
"""
...
def list_available_interferences() -> list[str]:
    ...
    return [
        d.name
        for d in TEST_SAMPLES_DIR.iterdir()
...
def get_sample_filepath(interference: str, data_type: str, drone: str) -> Path | None:
...
@st.cache_data
def load_sample_file(filepath: str) -> dict | None:
...
def create_iq_plot(iq_data: np.ndarray) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            x=x, y=i_data, mode="lines", name="I", line=dict(color="#e74c3c", width=1)
        )
    )
    fig.add_trace(
        go.Scatter(
            x=x, y=q_data, mode="lines", name="Q", line=dict(color="#2ecc71", width=1)
        )
    )
    fig.update_layout(
...
def create_psd_plot(psd_data: np.ndarray) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            y=psd_data, mode="lines", name="PSD", line=dict(color="#3498db", width=1.5)
        )
    )
    fig.update_layout(
...
def create_proba_chart(
    probabilities: np.ndarray, classes: list, true_idx: int, color: str
) -> go.Figure:
    ...
    def render(self):
        ...
        with st.sidebar:
            ...
            selected_type = st.selectbox(
                "Data Type", types, format_func=lambda x: f"{type_names[x]} ({x})"
            )
            ...
            sample_path = get_sample_filepath(
                selected_interference, selected_type, selected_drone
            )
            ...
            sample_idx = st.number_input(
                "Sample Index", min_value=0, max_value=n_samples - 1, value=0
            )
            ...
            if not available_models:
                ...
                run_inference = st.button(
                    "Run Inference", type="primary", width="stretch"
                )
        ...
        with col2:
            ...
            st.markdown(f"**Interference:** {interference}")
            st.markdown(f"**Protocol:** {drone_info.get('protocol', 'N/A')}")
        ...
        if run_inference and selected_model:
            ...
            with st.expander("How are these probabilities computed?"):
                if selected_model in ("VGG16", "ResNet50", "RFUAVNet"):
                    st.info(
                        "**Softmax over logits.** The final fully-connected layer "
                        "outputs one raw score (logit) per class. The softmax function "
                        "converts these scores into a probability distribution that "
                        "sums to 1. Higher logit = higher probability."
                    )
                else:
                    st.info(
                        "**Platt scaling.** The SVM decision function produces a signed "
                        "distance to each class boundary. A sigmoid calibration (Platt "
                        "scaling) converts these distances into probability estimates. "
                        "The result is a calibrated distribution over all classes."
                    )
