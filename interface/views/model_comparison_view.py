...
from settings import COLORS, NUM_CLASSES, MODELS_DIR


COMPARE_CSV_PATH = MODELS_DIR / "model_comparison_results.csv"
...
class ModelComparisonView:
    ...
    def render(self):
        ...
        if self.df.empty:
            ...
            st.info(
                "Expected columns: Model, Features, Accuracy, F1-Score, "
    ...
    def _render_accuracy_chart(self):
        ...
        for _, row in self.df.iterrows():
            plot_data.append(
                {"Model": row["Model"], "Metric": "Accuracy", "Value": row["Accuracy"]}
            )
            plot_data.append(
                {"Model": row["Model"], "Metric": "F1-Score", "Value": row["F1-Score"]}
            )
    ...
    def _render_detailed_table(self):
        ...
        display_df["Inference_p50_ms"] = display_df["Inference_p50_ms"].apply(
            lambda x: f"{x:.2f}"
        )
        display_df["Inference_p95_ms"] = display_df["Inference_p95_ms"].apply(
            lambda x: f"{x:.2f}"
        )
        display_df["Inference_p99_ms"] = display_df["Inference_p99_ms"].apply(
            lambda x: f"{x:.2f}"
        )
        display_df["Model_Size_MB"] = display_df["Model_Size_MB"].apply(
            lambda x: f"{x:.1f}"
        )
        ...
        st.caption(
            "p50/p95/p99: inference time percentiles over 100 single-sample runs "
            "(10 warmup runs excluded). Measured on Colab A100 GPU."
        )
    ...
    def _render_architecture_comparison(self):
        ...
        st.caption(
            "Architecture visualization is limited to PyTorch models. "
            "SVM (scikit-learn) has no layer structure to display."
        )
