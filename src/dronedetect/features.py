...
def compute_psd(
    ...
    freqs, psd = welch(
        segment, fs=fs, nperseg=nfft, window=window, return_onesided=False
    )
...
def compute_spectrogram(
    ...
    - Two-sided spectrum with fftshift for complex IQ signals
    ...
    # Compute spectrogram (two-sided for complex IQ signals)
    ...
    # Shift frequency axis to [-fs/2, +fs/2] (negative freqs carry distinct info for IQ)
    ...
    # Convert magnitude to dB: 20*log10(mag) since power = mag²
    spec_db = 20 * np.log10(spec_resized + 1e-12)
